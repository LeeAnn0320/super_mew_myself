...
def _merge_to_parent_level(docs:List[Dict],threshold:int=2)->Tuple[List[Dict],int]:
    ...
    '''
    defaultdict是一个特殊字典 
    defaultdict(list)表示当访问的键不存在时会创建一个空列表
    '''
    for doc in docs:
    ...
    parent_docs=_parent_chunk_store.get_document_by_ids(merge_parent_ids)
...
def _auto_merge_documents(docs:List[Dict],top_k:int)->Tuple[List[Dict],Dict[str,any]]:
    ...
    replaced_count=merged_count_l2_l1+merged_count_l3_l2
...
def retrive_documents(query:str,top_k:int=5)->Dict[str,Any]:
    ...
    try:
        ...
        retrived=_milvus_manager.hybrid_retrieve(
        ...
        reranked,rerank_meta=_rerank_documents(query=query,docs=retrived,top_k=top_k)
        ...
        '''
        update()操作是将另一个字典的键值对合并过来，独有的键就是直接添加，相同的键就是值会覆盖
