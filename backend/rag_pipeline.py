...
def retrieve_initial(state:RAGState)->RAGState:
    ...
    retrieved=retrieve_documents(query=query,top_k=5)
    results=retrieved.get("docs",[])
    retrieve_meta=retrieved.get("meta",{})
    context=_format_docs(docs=results)
    emit_rag_step(
        "🧱",
        "三级分块检索",
        (
...
def grade_documents_node(state:RAGState)->RAGState:
    ...
    if not grader:
        grader_update={
            "grader_score":"unknown",
            "grader_route":"rewrite_question",
            "rewrite_needer":True
        }
        rag_trace=state.get("rag_trace",{})or {}
        rag_trace.update(grader_update)
    ...
    if route=="generate_answer":
        ...
        emit_rag_step("❗","文档相关性不足,将重写查询",f"评分:{score}")

    grader_update={
        "grader_score":score,
        "grader_route":route,
        "rewrite_needer":route=="rewrite_question"
    }
    rag_trace=state.get("rag_trace",{})or {}
    rag_trace.update(grader_update)
...
def rewrite_question_node(state:RAGState)->RAGState:
    ...
    if strategy in ("step_back","complex"):
        emit_rag_step("🧠","生成退步问题",f"使用策略:{strategy}")
    ...
    if strategy in ("hyde","complex"):
        emit_rag_step("📋","假设性文档生成中")
        hypothetical_doc=generate_hypothetical_document(query=question)
...
def retrieve_expanded(state:RAGState)->RAGState:
    ...
    emit_rag_step("🔁","正在进行查询扩展重新搜索",f"策略:{strategy}")
    results:List[Dict]=[]
